...
export default function DocumentLibraryScreen() {
  ...
  if (searchVisible) return <View style={styles.page}>
    ...
    <ScrollView contentContainerStyle={styles.searchContent} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
      {!searchQuery.trim() ? <Text style={styles.searchHint}>输入关键词，搜索文件名、保存位置或正文内容</Text> : searchResults.map((document) => <SearchResultRow key={document.id} document={document} onPress={() => router.push({ pathname: "/(tabs)/doc-reader", params: { id: document.id } })} />)}
      {searchQuery.trim() && searchResults.length === 0 ? <Text style={styles.searchEmpty}>没有找到相关文档</Text> : null}
    </ScrollView>
  </View>;

  if (folder) return <View style={styles.page}>
    <View style={styles.folderHeader}><BackButton onPress={() => router.back()} /><Text numberOfLines={1} style={styles.folderTitle}>{folder.split("/").pop()}</Text><View style={styles.folderActions}><TouchableOpacity accessibilityLabel="刷题" style={styles.folderAction} onPress={() => Alert.alert("刷题", "该文件夹暂无可开始的题目")}><Play size={20} color={colors.learning} fill={colors.learning} /></TouchableOpacity>{selectedDocumentIds.size === 0 ? <TouchableOpacity accessibilityLabel="搜索文档" style={styles.folderAction} onPress={() => setSearchVisible(true)}><Search size={20} color={colors.text} /></TouchableOpacity> : null}<TouchableOpacity accessibilityLabel="更多" style={styles.folderAction} onPress={() => Alert.alert("更多", "更多操作将在下一阶段开放")}><MoreHorizontal size={21} color={colors.text} /></TouchableOpacity></View></View>
    {selectedDocumentIds.size > 0 ? <DocumentSelectionBar count={selectedDocumentIds.size} onCancel={exitSelection} onDelete={confirmBatchDelete} /> : null}
  ...
  return <View style={styles.page}>
    <View style={styles.header}><BackButton onPress={() => router.back()} /><Text numberOfLines={1} style={styles.title}>文档库</Text><View style={styles.actions}>{selectedDocumentIds.size === 0 ? <TouchableOpacity accessibilityLabel="搜索文档" onPress={() => setSearchVisible(true)}><Search size={22} color={colors.text} /></TouchableOpacity> : null}<FilePicker onFileSelected={handleFileSelected} label="＋" iconOnly /></View></View>
    {selectedDocumentIds.size > 0 ? <DocumentSelectionBar count={selectedDocumentIds.size} onCancel={exitSelection} onDelete={confirmBatchDelete} /> : null}
