...
export const markdownCss = `
  ...
  body, body * { font-family: BaguMiSans, sans-serif !important; color: #171824; } body { margin: 0; font-size: 16px; line-height: 1.66; background: transparent; word-break: break-word; }
  ...
  blockquote { margin: .9em 0; padding: .1em 1em; border-left: 3px solid #285ec9; background: #f7f8fa; }
  pre { overflow-x: auto; padding: .85em; border-radius: 8px; background: #f2f4f8; color: #171824 !important; }
  code { font-family: inherit !important; font-size: .88em; background: #f2f4f8; color: #171824 !important; border-radius: 4px; padding: .12em .28em; }
...
export function renderMarkdownHtml(markdown: string, fontUri = "") {
  const fontFace = fontUri ? `@font-face { font-family: BaguMiSans; src: url('${fontUri}'); }` : "";
  return `<!doctype html><html><head><meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1"><style>${fontFace}${markdownCss}</style></head><body>${renderMarkdownBody(markdown)}</body></html>`;
}
