...
function renderPreviewMarkdown(markdown: string, color: string) {
  ...
  return lines.map((line, index) => {
    ...
    const previewSection = section(value);
    if (previewSection) {
      if (!previewSection.detail && !hasSectionContent(index)) return null;
      return <Text key={`section-${index}`} style={[styles.previewText, { color }]}><Text style={styles.previewSectionTitle}>{previewSection.label}{previewSection.detail ? "：" : ""}</Text>{previewSection.detail ? inline(previewSection.detail) : null}</Text>;
    }
    ...
    if (value.startsWith("- ") || value.startsWith("* ")) return <View key={`bullet-${index}`} style={{ flexDirection: "row", gap: 8, marginBottom: 3 }}><Text style={{ color, fontSize: 17, lineHeight: 26 }}>•</Text><Text style={[styles.previewText, { color, flex: 1 }]}>{inline(value.slice(2))}</Text></View>;
    if (value.startsWith("> ")) return <View key={`quote-${index}`} style={{ borderLeftWidth: 3, borderLeftColor: colors.primary, paddingLeft: 12, marginVertical: 3 }}><Text style={[styles.previewText, { color }]}>{inline(value.slice(2))}</Text></View>;
    return <Text key={`text-${index}`} style={[styles.previewText, { color }]}>{inline(value)}</Text>;
  });
}
  ...
  return <View style={[styles.page, { backgroundColor: isDark ? colors.bgDark : colors.bg }]}>
    ...
    <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
      ...
      <View style={styles.tagArea}>
        {(draft.tags?.length ?? 0) > 0 ? draft.tags?.map((tag) => <TouchableOpacity key={tag} style={[styles.tagChip, { backgroundColor: isDark ? "#3A3023" : "#FFF4DF" }]} onPress={() => removeTag(tag)}><Text style={styles.tagChipText}>{tag}</Text><X size={13} color={colors.primary} strokeWidth={2.5} /></TouchableOpacity>) : <Text style={[styles.emptyTags, { color: colors.textTertiary }]}>尚未添加标签</Text>}
      ...
      <TextInput style={[styles.editor, styles.contentCard, styles.questionEditor, { backgroundColor: isDark ? "#2E3038" : "#F7F8FA", color: textColor }]} value={draft.q} onChangeText={(q) => setDraft({ ...draft, q })} placeholder="输入题目" placeholderTextColor={colors.textTertiary} multiline textAlignVertical="top" />
      <View style={styles.answerHeader}><Text style={[styles.label, styles.answerLabel, { color: colors.textSecondary }]}>答案</Text><View style={[styles.previewToggle, { backgroundColor: isDark ? "#2E3038" : "#F1F2F5" }]}><TouchableOpacity style={[styles.toggleOption, !answerPreview && styles.toggleActive, !answerPreview && { backgroundColor: isDark ? colors.surfaceDark : "#fff" }]} onPress={() => setAnswerPreview(false)}><Text style={[styles.toggleText, !answerPreview && { color: textColor }]}>编辑</Text></TouchableOpacity><TouchableOpacity style={[styles.toggleOption, answerPreview && styles.toggleActive, answerPreview && { backgroundColor: isDark ? colors.surfaceDark : "#fff" }]} onPress={() => setAnswerPreview(true)}><Text style={[styles.toggleText, answerPreview && { color: textColor }]}>预览</Text></TouchableOpacity></View></View>
      {answerPreview ? <View style={[styles.previewCard, { backgroundColor: isDark ? "#2E3038" : "#F7F8FA" }]}>{renderPreviewMarkdown(draft.a || "暂无答案内容", textColor)}</View> : <TextInput style={[styles.editor, styles.contentCard, styles.answerEditor, { backgroundColor: isDark ? "#2E3038" : "#F7F8FA", color: textColor, height: answerHeight }]} value={draft.a} onChangeText={(a) => setDraft({ ...draft, a })} onContentSizeChange={(event) => setAnswerHeight(Math.max(172, event.nativeEvent.contentSize.height + 32))} placeholder="支持 Markdown 和 LaTeX：$...$、$$...$$" placeholderTextColor={colors.textTertiary} multiline textAlignVertical="top" />}
    </ScrollView>
    ...
    <Modal visible={creationMode !== null} transparent animationType="fade" onRequestClose={() => setCreationMode(null)}>
      <View style={styles.creationOverlay}>
        <View style={[styles.creationDialog, { backgroundColor: surface }]}>
          ...
          <TextInput style={[styles.creationInput, { backgroundColor: isDark ? "#2E3038" : colors.surface, color: textColor, borderColor }]} value={newTagName} onChangeText={setNewTagName} placeholder="输入标签名称" placeholderTextColor={colors.textTertiary} autoFocus returnKeyType="done" onSubmitEditing={submitNewTag} />
