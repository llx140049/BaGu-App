    ...
    <Modal visible={visible} transparent animationType="slide" onRequestClose={onClose}>
      <View style={styles.overlay}>
        <View style={[styles.sheet, { backgroundColor: surface }]}>
          ...
          <TextInput style={[styles.input, { color: textColor, borderColor }]} value={draft.cat} onChangeText={(cat) => setDraft({ ...draft, cat })} placeholder="分类，例如：JavaScript" placeholderTextColor={colors.textTertiary} />
          <TextInput style={[styles.input, styles.multiline, { color: textColor, borderColor }]} value={draft.q} onChangeText={(q) => setDraft({ ...draft, q })} placeholder="题目" placeholderTextColor={colors.textTertiary} multiline />
          <TextInput style={[styles.input, styles.multiline, { color: textColor, borderColor }]} value={draft.a} onChangeText={(a) => setDraft({ ...draft, a })} placeholder="答案" placeholderTextColor={colors.textTertiary} multiline />
          <View style={styles.actions}>
            {question?.id && onDelete ? <TouchableOpacity style={styles.deleteButton} onPress={onDelete}><Text style={styles.deleteText}>删除</Text></TouchableOpacity> : null}
            <View style={styles.rightActions}>
              <TouchableOpacity style={styles.button} onPress={onClose}><Text style={[styles.cancelText, { color: colors.textSecondary }]}>取消</Text></TouchableOpacity>
              <TouchableOpacity style={[styles.button, styles.saveButton]} onPress={save}><Text style={styles.saveText}>保存</Text></TouchableOpacity>
            </View>
          </View>
        </View>
      </View>
