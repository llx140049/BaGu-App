...
export default function StudyScopeBottomSheet({ visible, rangeLabel, options, onClose, onSelect }: StudyScopeBottomSheetProps) {
  ...
  return <Modal visible={visible} transparent animationType="fade" onRequestClose={onClose}>
    <TouchableOpacity activeOpacity={1} style={styles.overlay} onPress={closeWithAnimation}>
...
const styles = StyleSheet.create({
  ...
  title: { color: colors.text, fontSize: 22, fontWeight: "600" }, range: { color: colors.textSecondary, fontSize: 14, marginTop: 5, marginBottom: 12 },
  ...
  rowDisabled: { opacity: 0.35 }, rowPressed: { backgroundColor: colors.primaryLight }, rowLabel: { flex: 1, color: colors.text, fontSize: 16 }, count: { color: colors.textSecondary, fontSize: 15, fontVariant: ["tabular-nums"] }, textDisabled: { color: colors.textTertiary },
});
